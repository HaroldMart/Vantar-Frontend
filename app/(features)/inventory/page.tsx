...
export default function InventoryPage() {
  ...
  const [deleteInventoryId, setDeleteInventoryId] = useState<number | null>(
    null
  );
  ...
  const updateInventory = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_URL}/${editInventory.id}`,
        updatedInventory
      );

      setInventories(
        inventories.map((inv) =>
          inv.id === editInventory.id ? response.data : inv
        )
      );
