...
export const ProductsTable: React.FC<ProductsTableProps> = ({ businessId }) => {
  ...
  const fetchProducts = async () => {
    try {
      const products = await productApiService.getAllProducts(businessId);
  ...
  const addProduct = async (e: React.FormEvent) => {
    ...
    try {
      const createdProduct = await productApiService.createProduct(
        businessId,
        newProductData
      );
      if (!createdProduct) {
  ...
  const deleteProduct = async () => {
    ...
    try {
      await productApiService.deleteProduct(businessId, productToDelete.id);
      setProducts(
        ...
        <div className="flex gap-2 w-full">
              ...
              <div className="flex w-[450px]">
                ...
                <div className="relative w-full">
                  ...
                  <button
                    type="submit"
                    disabled
                    className="absolute h-full top-0 end-0 p-2.5 text-sm font-medium text-white bg-blue-700 rounded-e-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300"
                  >
                    <BiSearch size={18} />
          ...
          {isFiltersOpen && (
            <div className="flex gap-2 w-full">
              ...
              <div className="relative w-full max-w-40">
                <label htmlFor="accessories" className="sr-only">
            ...
            {[...Array(totalPages)].map((_, index) => (
              <li key={index}>
                <button
                  ...
                  className={`flex items-center justify-center px-3 h-8 leading-tight transition-all duration-100 border-gray-300 ${
                    currentPage === index + 1
